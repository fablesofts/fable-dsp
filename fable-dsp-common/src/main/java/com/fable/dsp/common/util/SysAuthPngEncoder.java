 ...
 * be saved as a PNG file. The Image is presumed to use the DirectColorModel.
 * <p>
 * Thanks to Jay Denny at KeyPoint Software http://www.keypoint.com/ who let
 * me develop this code on company time.
 * </p>
 * <p>
 ...
 * Inc., 59 Temple Place, Suite 330, Boston, MA 02111-1307 USA A copy of the
 * GNU LGPL may be found at
 * <code>http://www.gnu.org/copyleft/lesser.html</code>
 * </p>
 * 
 * @author J. David Eisenberg
 * @version 1.5, 19 Oct 2003 CHANGES: -------- 19-Nov-2002 : CODING STYLE
 *          CHANGES ONLY (by David Gilbert for Object Refinery Limited);
 *          19-Sep-2003 : Fix for platforms using EBCDIC (contributed by
 *          Paulo Soares); 19-Oct-2003 : Change private fields to protected
 *          fields so that PngEncoderB can inherit them (JDE) Fixed bug with
 *          calculation of nRows
 */
public class SysAuthPngEncoder extends Object {
     ...
     * Class constructor
     */
    public SysAuthPngEncoder() {
        this(null, false, SysAuthPngEncoder.FILTER_NONE, 0);
    }
     ...
     * @see java.awt.Image
     */
    public SysAuthPngEncoder(final Image image) {
        this(image, false, SysAuthPngEncoder.FILTER_NONE, 0);
    }
     ...
     * @see java.awt.Image
     */
    public SysAuthPngEncoder(final Image image, final boolean encodeAlpha) {
        this(image, encodeAlpha, SysAuthPngEncoder.FILTER_NONE, 0);
    }
    ...
    public void setFilter(final int whichFilter) {
        this.filter = SysAuthPngEncoder.FILTER_NONE;
        if (whichFilter <= SysAuthPngEncoder.FILTER_LAST)
     ...
     * @return Array of newly desired length. If shorter than the original,
     *         the trailing elements are truncated.
     */
    protected byte[]
        resizeByteArray(final byte[] array, final int newLength) {
    ...
    /**
     * Write an array of bytes into the pngBytes array. Note: This routine
     * has the side effect of updating maxPos, the largest element written in
     * the array. The array is resized by 1000 bytes or the length of the
    ...
    protected int writeBytes(final byte[] data, final int offset) {
        ...
        if (data.length + offset > this.pngBytes.length)
            this.pngBytes =
                this.resizeByteArray(this.pngBytes, this.pngBytes.length +
     ...
     * maxPos, the largest element written in the array. The array is resized
     * by 1000 bytes or the length of the data to be written, whichever is
     * larger.
     * 
     * @param data
     *        The data to be written into pngBytes.
    ...
    protected void writeHeader() {
        ...
        this.bytePos = this.writeBytes(SysAuthPngEncoder.IHDR, this.bytePos);
        this.width = this.image.getWidth(null);
        this.height = this.image.getHeight(null);
        ...
        this.bytePos =
            this.writeByte((this.encodeAlpha) ? 6 : 2, this.bytePos); // direct
                                                                      // model
     ...
     * leftBytes to store the original values of the previous pixels. The
     * array is 16 bytes long, which will easily hold two-byte samples plus
     * two-byte alpha.
     * 
     * @param pixels
    ...
    protected void filterSub(final byte[] pixels, final int startPos,
        ...
        for (i = actualStart; i < startPos + nBytes; i++) {
            ...
            pixels[i] =
                (byte)((pixels[i] - this.leftBytes[leftExtract]) % 256);
    ...
    /**
     * Perform "up" filtering on the given row. Side effect: refills the
     * prior row with current row
     * 
     * @param pixels
     *        The array holding the scan lines being built
     * @param startPos
    ...
    protected void filterUp(final byte[] pixels, final int startPos,
        ...
        for (i = 0; i < nBytes; i++) {
            ...
            pixels[startPos + i] =
                (byte)((pixels[startPos + i] - this.priorRow[i]) % 256);
            this.priorRow[i] = currentByte;
        }
    }

    /**
     * Write the image data into the pngBytes array. This will write one or
     * more PNG "IDAT" chunks. In order to conserve memory, this method grabs
     * as many rows as will fit into 32K bytes, or the whole image; whichever
     * is less.
     * 
    ...
    protected boolean writeImageData() {
        ...
        final DeflaterOutputStream compBytes =
            new DeflaterOutputStream(outBytes, scrunch);
        try {
            while (rowsLeft > 0) {
                nRows =
                    Math.min(
                        32767 / (this.width * (this.bytesPerPixel + 1)),
                        rowsLeft);
                nRows = Math.max(nRows, 1);
                final int[] pixels = new int[this.width * nRows];
                pg =
                    new PixelGrabber(this.image, 0, startRow, this.width,
                 ...
                 */
                scanLines =
                    new byte[this.width * nRows * this.bytesPerPixel + nRows];
                if (this.filter == SysAuthPngEncoder.FILTER_SUB)
                    this.leftBytes = new byte[16];
                if (this.filter == SysAuthPngEncoder.FILTER_UP)
                    this.priorRow =
                        new byte[this.width * this.bytesPerPixel];
                scanPos = 0;
                startPos = 1;
            ...
            this.bytePos = this.writeBytes(SysAuthPngEncoder.IDAT, this.bytePos);
            this.crc.update(SysAuthPngEncoder.IDAT);
            this.bytePos =
                this.writeBytes(compressedLines, nCompressed, this.bytePos);
    ...
    protected void writeEnd() {
        ...
        this.bytePos = this.writeBytes(SysAuthPngEncoder.IEND, this.bytePos);
        this.crc.reset();
        this.crc.update(SysAuthPngEncoder.IEND);
        this.crcValue = this.crc.getValue();
        this.bytePos = this.writeInt4((int)this.crcValue, this.bytePos);
    }
}
