...
import com.fable.dsp.common.exception.ConnectException;
import com.fable.dsp.common.exception.CreateAddTableException;
import com.fable.dsp.common.exception.CreateTriggerException;
import com.fable.dsp.common.util.DataBaseTypes;
import com.fable.dsp.service.dataswitch.intf.NetWorkCommunicationService;

@Service
public class AddDBTable {

    private static final Logger logger  = LoggerFactory.getLogger(AddDBTable.class);
    ...
    public void createAddTable(AddTableDto dto) throws SQLException, ClassNotFoundException {
        ...
        NetWorkCommunicationService impl = new NetWorkCommunicationServiceImpl();
        ...
        if  (DataBaseTypes.ORACLE.equals(dto.getDbtype())) {
            try {
                ...
                throw new CreateAddTableException(e1);
            } 
        }
                ...
                if(colarr!=null && colarr.length!=0){
                    ...
                    if (!flag) {
                        ...
                        try {
                            ...
                            throw new CreateAddTableException(e);
                        }
                    }
                }
            }
                ...
                if(!isTrigger(dsdto,tablenames[x])){
                   ...
                   //拼接delete触发器后部 
                    ...
                    try {
                        ...
                        throw new CreateTriggerException(e);
                    }
                    try {
...
//                            }
    ...
    public Map<String,String> getKeyType(DataSourceDto dsdto,String tableName) {
        ...
        rs = getConnection(dsdto).getMetaData().getColumns(null, null, tableName,null);
            ...
            logger.error("get column value type or size failed the error is : {}",e.getMessage());
            throw new CreateTriggerException(e);
        }finally{
            close();
        }
...
//                    .append("'");
        ...
        try {
            ...
            logger.error("select Table failed the error is : {}",e.getMessage());
            throw new CreateAddTableException(e);
        }
        if(tableList.size()>0){
            return true;
    ...
    public void dropTrigger(DataSourceDto ds,String tablename){
        ...
        if (DataBaseTypes.DM7.equals(ds.getDb_type())) {
            ...
            try {
                ...
                logger.error("the trigger drop failed the error is : {}",e.getMessage());
                throw new CreateTriggerException(e);
            }
            catch (ClassNotFoundException e) {
                logger.error("the trigger drop failed the error is : {}",e.getMessage());
    ...
    public String[] getPremaryKey (DataSourceDto dsdto,String tablename) {
        ...
        try {
            ...
            logger.error("get PremaryKey error the error is : {}",e.getMessage());
            throw new CreateAddTableException(e);
        }finally {
            close();
        }
    ...
    private Connection getConnection(DataSourceDto ds) {
        ...
        try {
            ...
            logger.error("get Connection error the error is : {}",e.getMessage());
            throw new ConnectException(e);
        }
        try {
            try {
