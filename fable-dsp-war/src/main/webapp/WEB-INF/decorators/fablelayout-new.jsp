...
<body class="easyui-layout" >
	...
	<%--系统授权窗体sysAuthWindow --%>
	<div id="sysNoAuthWindow" data-options="iconCls:'icon-hidden',resizable:false,modal:true,closed:true" class="easyui-dialog" title="关于授权"
		style="width:420px;height:220px;padding:10px" >
		<form id="sysAuthForm">
			<table>
				<tr align="center">
					<td align="center" style="font-family: 微软雅黑; color: #555555; font-size: 14px;">
						系统尚未获得授权</td>
				</tr>
			</table>
		</form>
	</div>
	<div id="sysAuthWindow" data-options="iconCls:'icon-hidden',resizable:false,modal:true,closed:true" class="easyui-dialog" title="关于授权"
		style="width:420px;height:320px;padding:10px" >
		<form id="sysAuthForm">
			<table>
				<tr>
					<td><img src="${pageContext.request.contextPath}/sysauth/viewLicence"></td>
				</tr>
			</table>
		</form>
	</div>
				...
				<div class="userMsg">
					...
					<div class="userMsgSec">
						<ul>
							...
							<li><a href="javascript:void(0)" id="showAuthInfo" onclick="ShowAuthInfo()"><img src="${pageContext.request.contextPath}/style/images/main/img_1.gif" align="absmiddle" /> &nbsp; 关于授权</a></li>
	...
	$(function(){
		$.ajax({
			type:'post',
			url: '${pageContext.request.contextPath}/sysauth/aboutLicence',
	        success: function callback(data) {
	        	var remindData = parseInt(data);
	    		if(data == null || data == ""){
	    			$.messager.show({
        				title:'温馨提示',
        				msg:'系统尚未获得授权',
	    			...
	    			}else if(remindData <= 0){
	    				var lastData = remindData + 30;
	    				$.messager.show({
	        				title:'温馨提示',
	        				msg:'系统授权已经到期，系统将会继续为您提供' + lastData + '天的服务',
	        				timeout:10000,
	        			});
	        		} else if(remindData <= 30){
	        			$.messager.show({
	        				title:'温馨提示',
	        				msg:'据授权到期日期还剩：' + remindData + '天',
	        				timeout:10000,
	        			});
	        		}
	    		}
	...
	function ShowAuthInfo(){
		$.ajax({
            ...
            success: function callback(data) {
            	var remindData = parseInt(data);
        		if(data == null || data == ""){
					$('#sysNoAuthWindow').dialog('open');
        		} else {
        			if(remindData <= 0){
        				$('#sysNoAuthWindow').dialog('open');
            		} else {
            			$('#sysAuthWindow').dialog('open');
            		}
        		}
        	}
        });
